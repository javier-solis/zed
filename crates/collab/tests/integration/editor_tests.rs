...
async fn test_lsp_document_links(cx_a: &mut TestAppContext, cx_b: &mut TestAppContext) {
    ...
    let other_target = other_uri.to_string();
    ...
    let other_uri_for_resolve = other_uri.clone();
    ...
    let other_editor = workspace_b
        ...
        .downcast::<Editor>()
        .unwrap();
    other_editor.read_with(cx_b, |editor, cx| {
        let buffer = editor.buffer().read(cx).as_singleton().unwrap();
        assert_eq!(
            buffer.read(cx).text(),
            other_contents,
